...
export default function Portfolio(props) {
    ...
    return (
        <div className={props.shrink ? `${styles.shrunk} ${styles.portfolio}` : styles.portfolio}>
            <h1 className={styles.header}>Work I've Done</h1>
            <hr className={styles.titlehr} />
            <div className={styles.projects}>
                <Project title="Unbeatable Tic-Tac-Toe" picture={tictactoe} description={<p>An unbeatable tic tac toe game built with the minimax algorithm using:</p>} technologies={<p>{tech.node} + {tech.react}</p>} srcControlLink='https://github.com/jldoan91/tictactoe' srcControl="GitHub" hostLink='https://jldoantictactoe.herokuapp.com/' host="Heroku" />
                <Project title="Contact List" picture={contactList} description={<p>A persistent contact list app that utilizes the browser's local storage to hold on to contact data. <br /> Built using:</p>} technologies={<p>{tech.jquery}</p>} host="CodePen" hostLink='https://codepen.io/jldoan91/full/PgprvB' />
            </div>
        </div>
    )
}
